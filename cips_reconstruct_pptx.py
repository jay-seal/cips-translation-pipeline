...
Usage:
    python cips_reconstruct_pptx.py \
...
def reconstruct(pptx_path: Path, json_path: Path, output_path: Path,
    ...
    for seg in segments:
        ...
        # Decode literal \n (backslash+n) to actual newlines via the shared
        # helper, so decoding behaves identically across all formats.
        source_text = decode_newlines(source_text)
        translated = decode_newlines(translated)
